...
fdescribe('Players', () => {
  describe('Base Player', () => {
    ...
    it('Prepares for a new round', () => {
      // SET UP
      player.addShip(ShipType.DESTROYER, { row: 0, col: 0 }, Direction.Right);
      player.receiveAttack({ row: 0, col: 0 });
      player.receiveAttack({ row: 0, col: 0 });
      player.receiveAttack({ row: 4, col: 8 });
      player.receiveAttack({ row: 4, col: 3 });
      player.addWin();
      ...
      player.getBoardCopy().forEach((row) => {
        row.forEach((cell) => {
          expect(cell.hit).toBeFalse();
          expect(cell.shipInfo).toBeNull();
        });
      });
    });

    it('Prepares for a new game', () => {
      // SET UP
      player.addShip(ShipType.DESTROYER, { row: 0, col: 0 }, Direction.Right);
      player.receiveAttack({ row: 0, col: 0 });
      player.receiveAttack({ row: 0, col: 0 });
